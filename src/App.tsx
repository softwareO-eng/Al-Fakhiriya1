...
export default function App() {
  ...
  const expiringDocsCount = documents.filter(d => d.daysUntilExpiry <= 30).length;
      ...
      <aside className={`absolute md:relative z-50 md:z-auto w-64 h-full bg-neutral-950 border-r border-neutral-800 flex flex-col transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-200 ease-in-out`}>
        ...
        <div className="p-4 border-t border-neutral-800">
          <div className="flex items-center space-x-3">
            <div className="h-10 w-10 rounded-full bg-indigo-600 flex items-center justify-center text-sm font-medium text-white shadow-inner">
              AU
            </div>
            <div>
              <p className="text-sm font-medium text-white">Admin User</p>
              <p className="text-xs text-emerald-400 font-medium tracking-wide">✓ Local Mode</p>
            </div>
          </div>
        </div>
      </aside>
